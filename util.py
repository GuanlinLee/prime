...
def save_video_frames(video_path, mid_name, img_size=(512,512)):
    ...
    os.makedirs(f'data/{mid_name}/{video_name}', exist_ok=True)
    for i in range(len(video)):
        ind = str(i).zfill(5)
        image = T.ToPILImage()(video[i])
        image_resized = image.resize((img_size),  resample=Image.Resampling.LANCZOS)
        image_resized.save(f'data/{mid_name}/{video_name}/{ind}.png')
